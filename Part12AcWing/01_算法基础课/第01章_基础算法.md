  ...
  + 输出每个a[i]时：`a[i] = p[i] + a[i-1];`
    ...
    public class Main {
        ...
        public static void main(String[] args) {
            ...
            //进行n次插入，初始化差分数组
            for (int i = 1; i <= n; i++) insert(p, i, i, a[i]);

            while (m-- > 0) {
                int l = sc.nextInt(), r = sc.nextInt(), c = sc.nextInt();
                insert(p, l, r, c);
            }
            //经过一系列插入操作后，现在答案数组应该是p数组的前缀和，直接在让b数组变成b的前缀和。
            int[] s = new int[n + 1];
            //公式 s[i] = s[i-1] + p[i] 
            for (int i = 1; i <= n; i++) s[i] = s[i - 1] + p[i]; // 这里体现了差分是前缀和的逆运算
...
## 五、双指针法
> 在数组里是用两个整型值代表下标，在链表里是两个指针，一般能实现O（n）的时间解决问题，两个指针的位置一般在`第一个元素和第二个元素`(相同方向)或者`第一个元素和最后一个元素`(相反方向)，快指针在前“探路”，当符合某种条件时慢指针向前挪。时间复杂度是`O(n)`
### 参考博客
...
### 实战
+ [AcWing 799.最长连续不重复子序列](https://www.acwing.com/problem/content/description/801/)：滑动窗口问题
  > 类似题目：[LeetCode 3.无重复字符的最长子串](https://leetcode-cn.com/problems/longest-substring-without-repeating-characters/)，只不过换成字符串了
    ...
    public class Main {
        public static void main(String[] args) {
            ...
            Map<Integer, Integer> map = new HashMap<>(); // 记录数组和其对应的频率
            // 滑动窗口核心算法
            int left = 0, right = 0, res = 0;
            while (right < n) {
                int numRight = a[right];
                right++;
                // 更新相关数据
                if (map.get(numRight) == null) map.put(numRight, 1); // 之前没有地话频率认为是1
                else map.put(numRight, map.get(numRight) + 1); // 频率 + 1
                
                while (map.get(numRight) > 1) { // 因为numRight的频率 > 1了，因此要一直删除左侧元素，直到numRight的频率变成1(即滑动窗口内无重复数字)
                    int numLeft = a[left]; // 最左侧数字
                    map.put(numLeft, map.get(numLeft) - 1);
                    left++;
                }
                
                // 退出上面的循环，[left, right)的区间肯定没有重复数字了，更新最大区间长度即可
...
+ [AcWing 800.数组元素的目标和](https://www.acwing.com/problem/content/description/802/)
    ...
    public class Main {
        ...
        private static int[] sum(int[] a, int[] b, int x) {
            ...
            while (left < a.length) {
                while (right >= 0 && a[left] + b[right] > x) right--;
                if (a[left] + b[right] == x) return new int[]{left, right};
                left++;
            }
            return new int[2];
        }
